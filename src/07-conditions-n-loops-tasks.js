...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const first = arr.indexOf(arr[i]);
    const last = arr.lastIndexOf(arr[i]);
    if (first === last) return arr[i];
  }
  return null;
}
...
function getDigitalRoot(num) {
  const sum = String(num).split('').reduce((sum1, cur) => sum1 + Number(cur), 0);
